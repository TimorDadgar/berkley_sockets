...
int Client::server_input(){
    uint32_t ipAddress = (127 << 24) | (0 << 16) | (0 << 8) | 1;
    char buffer[1300];

    // std::shared_ptr<BallState> recvBallState = std::make_shared<BallState>();
    BallState recvBallState;

    sockaddr_in fromAddr;
    int fromlen = sizeof(fromAddr);
    SocketAddress fromSocketAddr(ipAddress, 45000);
    ...
    int bytesReceived = UDPSocketRef->ReceiveFrom(&recvBallState, sizeof(recvBallState), fromSocketAddr);
    ...
    // std::string server_input_data(buffer, bytesReceived);
    std::cout << recvBallState.mPosX << std::endl;
...
int Client::start_pong_game(){
    ...
    while (!quit) {
        ...
        server_input();
        // std::cout << server_input_data << std::endl;
        // Move paddles with arrow keys
        const Uint8* keystates = SDL_GetKeyboardState(NULL);
        // if (keystates[SDL_SCANCODE_UP]) {

        if (server_input_data == "W") {
        // if (ClientInput == "W") {
            rightPaddleY -= 5;
        }
...
int main() {
    ...
    ClientInterface.berkley_socket();
    ClientInterface.start_pong_game();
    ClientInterface.clean_up();
    return 0;
}
